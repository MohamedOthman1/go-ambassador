...
type User struct {
	...
	Revenue *float64  `json:"revenue,omitempty" gorm:"-"` //omitEmpty remove the field if it is empty
}
...
func (user *User) ComparedPass(password string) error {
...
func (ambassador *Ambassador) CalculateRevenue(db *gorm.DB) {
	...
	for _, order := range orders {
		for _, orderItem := range order.OrderItems {
			revenue += orderItem.AmbassadorRevenu
		}
	}
